...
contributors_url = (
    "https://api.github.com/repos/{organization}/{repo}/contributors?page={page_no}"
)
...
def getRepos(org_name, top_n_repos, top_m_commiters, page_no):
    ...
    repos_json = requests.get(
        repo_url.format(organization=org_name, limit=PAGE_LIMIT, page_no=str(page_no)),
        headers=headers,
    ).json()
    ...
    if (
        "message" in repos_json
        or len(repos_json) == 0
        or page_no <= 0
        or int(top_n_repos) <= 0
        or int(top_m_commiters) < 0
    ):
        return "404", prev_page, next_page

    # Fetch the next page to get the details of the next page for the UI
    repos_json_next = requests.get(
        repo_url.format(
            organization=org_name, limit=PAGE_LIMIT, page_no=str(page_no + 1)
        ),
        headers=headers,
    ).json()
    ...
    if (
        "message" not in repos_json_next
        and len(repos_json_next["items"]) > 0
        and page_no * PAGE_LIMIT < int(top_n_repos)
    ):
        next_page = PAGE_URL.format(
            org_name, top_n_repos, top_m_commiters, str(nextpage_no)
        )
    if page_no > 1:
        prev_page = PAGE_URL.format(
            org_name, top_n_repos, top_m_commiters, str(prevpage_no)
        )
    ...
    for repo in repos_json["items"]:
        ...
        repos.append(
            Repository(
                repo["name"], repo["html_url"], repo["forks_count"], contributors
            )
        )
...
def get_contributors(org_name, repo, top_m_commiters):
    ...
    while cnt < int(top_m_commiters):
        contributors_json = requests.get(
            contributors_url.format(
                organization=org_name, repo=repo, page_no=str(page_no)
            ),
            headers=headers,
        ).json()
